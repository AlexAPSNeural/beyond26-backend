...
const transporter = nodemailer.createTransport({
  host: process.env.SMTP_HOST || 'beyond26advisors.com',
  port: parseInt(process.env.SMTP_PORT) || 465,
  secure: process.env.SMTP_SECURE === 'true' || true, // true for 465, false for other ports
  auth: {
    user: process.env.SMTP_USER || 'website@beyond26advisors.com',
    pass: process.env.SMTP_PASS || 'Beyond262026',
  },
  tls: {
...
<html>
  ...
  const mailOptions = {
    from: `"Beyond26 Website" <${process.env.EMAIL_FROM}>`,
    to: [process.env.EMAIL_TO_EDGAR, process.env.EMAIL_TO_ALEX].join(', '),
    replyTo: email,
...
export async function sendMeetingRequestEmail(meetingData) {
  const { advisors, name, email, phone, notes, selectedTimes } = meetingData;
...
${selectedTimes.map((time, index) => `${index + 1}. ${new Date(time).toLocaleString('en-US', { timeZone: 'America/Los_Angeles' })}`).join('\n')}
